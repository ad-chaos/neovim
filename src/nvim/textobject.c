  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (!vis_empty) {
    ...
    inside_quotes = col_start > 0
                    && (uint8_t)line[col_start - 1] == quotechar
                    && line[col_end] != NUL
                    && (uint8_t)line[col_end + 1] == quotechar;
  }
  ...
  if (old_col_start >= 0 && old_col_end > 0
      && (col_start <= 0 || col_end < 0 || (!vis_bef_curs && !vis_empty && !inside_quotes))) {
    col_start = old_col_start;
    col_end = old_col_end;
  } else if (col_end < 0 && old_col_start <= 0) {
    if (VIsual_active && *p_sel == 'e') {
  ...
  if ((include || (!vis_empty && inside_quotes)) && inc_cursor() == 2) {
    inclusive = true;
  }
  if (VIsual_active) {
